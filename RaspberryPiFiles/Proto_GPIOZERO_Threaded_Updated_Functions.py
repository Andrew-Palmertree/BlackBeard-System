...
def create_chute_thread():
	return threading.Thread(target=activate_chute)

def create_door_thread():
	return threading.Thread(target=activate_door)
...
def activate_door():
	"""
	Controls the door, unlocking and locking back after a specified amount of 
	time by the constant declared near the top of the file
	"""

	unlock_door()

	time.sleep(TIME_BETWEEN_LOCK_UNLOCK)
